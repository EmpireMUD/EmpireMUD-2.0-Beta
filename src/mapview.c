	...
	*self_icon = *mappc_icon = *map_icon = *veh_icon = *show_icon = *icon_color = '\0';
	...
	if (IS_BURNING(to_room)) {
		...
		if (PRF_FLAGGED(ch, PRF_POLITICAL | PRF_INFORMATIVE) && !show_dark && veh_is_shown) {
			...
			if (GET_LOYALTY(ch) && (GET_LOYALTY(ch) == ROOM_OWNER(to_room) || (!ROOM_OWNER(to_room) && find_city(GET_LOYALTY(ch), to_room))) && is_in_city_for_empire(to_room, GET_LOYALTY(ch), FALSE, &junk)) {
