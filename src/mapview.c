...
int pick_season(room_data *room) {
	...
	if (day_of_year < 6 * 30) {	// first half of year
		if (half_y >= round((day_of_year - 2) * a_slope)) {	// first winter line
			return northern ? TILESET_WINTER : TILESET_SUMMER;
		}
		else if (half_y >= round((day_of_year - 88) * b_slope)) {	// spring line
		...
		if (half_y >= round((day_of_year - 361) * -a_slope)) {	// second winter line
			return northern ? TILESET_WINTER : TILESET_SUMMER;
		}
