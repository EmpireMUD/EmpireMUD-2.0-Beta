		...
		if (!IS_NPC(c) && GET_ACTION(c) == ACT_BUILDING) {
			...
			if (!emp || GET_LOYALTY(c) == emp) {
				if (type && GET_CRAFT_ABILITY(type) != NO_ABIL) {
					...
					run_ability_hooks(c, AHOOK_ABILITY, GET_CRAFT_ABILITY(type), get_ability_level(c, GET_CRAFT_ABILITY(type)), NULL, NULL, NULL, room);
				}
			}
		}
	}
	...
	if (!IS_DISMANTLING(room) && BUILDING_RESOURCES(room)) {
		if ((res = get_next_resource(ch, BUILDING_RESOURCES(room), can_use_room(ch, room, GUESTS_ALLOWED), TRUE, &found_obj))) {
			...
			if (type && act_type == ACT_BUILDING && GET_CRAFT_ABILITY(type) != NO_ABIL) {
				...
				run_ability_hooks(ch, AHOOK_ABILITY, GET_CRAFT_ABILITY(type), get_ability_level(ch, GET_CRAFT_ABILITY(type)), NULL, NULL, NULL, room);
			}
		}
		else {
...
ACMD(do_lay) {
	...
	if (IS_NPC(ch)) {
		...
		if (SECT_FLAGGED(check_sect, SECTF_ROUGH)) {
			...
			run_ability_hooks(ch, AHOOK_ABILITY, ABIL_PATHFINDING, get_ability_level(ch, ABIL_PATHFINDING), NULL, NULL, NULL, IN_ROOM(ch));
		}
