	...
	if (ROOM_OWNER(IN_ROOM(ch)) && (ter = find_territory_entry(ROOM_OWNER(IN_ROOM(ch)), IN_ROOM(ch))) && ter->npcs) {
		*buf = '\0';
		LL_FOREACH(ter->npcs, npc) {
			if (npc->mob) {
				sprintf(buf + strlen(buf), "%s[%d] %s", *buf ? ", " : "", GET_MOB_VNUM(npc->mob), GET_SHORT_DESC(npc->mob));
			}
				...
				sprintf(buf + strlen(buf), "%s[%d] %s", *buf ? ", " : "", npc->vnum, nstr);
				free(nstr);
			}
			else {
				sprintf(buf + strlen(buf), "%s[%d] UNKNOWN", *buf ? ", " : "", npc->vnum);
			}
		}
