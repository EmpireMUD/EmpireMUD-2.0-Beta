...
ACMD(do_slow) {
	...
	// counterspell??
	if (trigger_counterspell(vict) || AFF_FLAGGED(vict, AFF_IMMUNE_MAGICAL_DEBUFFS)) {
		act("You try to use a slow hex on $N, but $E deflects it!", FALSE, ch, NULL, vict, TO_CHAR | TO_ABILITY);
		act("$n tries to hex you, but it's deflected by your counterspell!", FALSE, ch, NULL, vict, TO_VICT | TO_ABILITY);
		act("$n tries to hex $N, but $E deflects it.", FALSE, ch, NULL, vict, TO_NOTVICT | TO_ABILITY);
	}
	else {
		// succeed
	
		act("$N grows lethargic and starts to glow gray as you shout the slow hex at $M!", FALSE, ch, NULL, vict, TO_CHAR | TO_ABILITY);
		act("$n shouts something at you... The world takes on a gray tone and you become more lethargic.", FALSE, ch, NULL, vict, TO_VICT | TO_ABILITY);
		act("$n shouts some kind of hex at $N, who starts to move sluggishly and starts to glow gray!", FALSE, ch, NULL, vict, TO_NOTVICT | TO_ABILITY);
