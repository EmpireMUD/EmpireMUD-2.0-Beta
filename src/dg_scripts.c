/* ************************************************************************
*   File: dg_scripts.c                                    EmpireMUD 2.0b3 *
		...
		if (vd) {
			...
			switch (type) {
				...
				case VEH_TRIGGER: {
					...
					if ((c = get_char_by_vehicle(veh, name)) || (v = get_vehicle_by_vehicle(veh, name)) || (o = get_obj_by_vehicle(veh, name)) || (r = get_room(IN_ROOM(veh), name))) {
						// just setting vars
					}
					break;
				}
