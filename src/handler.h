...
#define affect_from_char_by_apply(ch, type, apply, show_msg)  affect_from_char_by_apply_and_caster((ch), (type), (apply), (show_msg), NULL)
void affect_from_char_by_bitvector_and_caster(char_data *ch, any_vnum type, bitvector_t bits, bool show_msg, char_data *caster);
#define affect_from_char_by_bitvector(ch, type, bits, show_msg)  affect_from_char_by_bitvector_and_caster(ch, type, bits, show_msg, NULL)
...
#define find_empire_vehicle_entry(emp, veh)  find_empire_vehicle_entry_by_id((emp), ((veh) ? VEH_IDNUM(veh) : NOTHING))
