...
int reduce_damage_from_skills(int dam, char_data *victim, char_data *attacker, int damtype) {
	...
	if (!self) {
		if (has_ability(victim, ABIL_NOBLE_BEARING) && check_solo_role(victim)) {
			...
			run_ability_hooks(victim, AHOOK_ABILITY, ABIL_NOBLE_BEARING, get_ability_level(victim, ABIL_NOBLE_BEARING), victim, NULL, NULL, NULL);
		}
		...
		if (damtype == DAM_MAGICAL && !IS_NPC(victim) && has_ability(victim, ABIL_NULL_MANA) && GET_MANA(victim) > 0) {
			...
			if (absorb > 0) {
				...
				run_ability_hooks(victim, AHOOK_ABILITY, ABIL_NULL_MANA, get_ability_level(victim, ABIL_NULL_MANA), attacker, NULL, NULL, NULL);
			}
		}
	}
	...
	if (!success) {
		...
		if (can_gain_skill) {
			...
			if (!IS_NPC(ch) && has_ability(ch, ABIL_DAGGER_MASTERY) && weapon && match_attack_type(GET_WEAPON_TYPE(weapon), TYPE_STAB)) {
				...
				run_ability_hooks(ch, AHOOK_ABILITY, ABIL_DAGGER_MASTERY, get_ability_level(ch, ABIL_DAGGER_MASTERY), victim, NULL, NULL, NULL);
			}
			...
			if (!IS_NPC(ch) && has_ability(ch, ABIL_STAFF_MASTERY) && weapon && TOOL_FLAGGED(weapon, TOOL_STAFF)) {
				...
				run_ability_hooks(ch, AHOOK_ABILITY, ABIL_STAFF_MASTERY, get_ability_level(ch, ABIL_STAFF_MASTERY), victim, NULL, NULL, NULL);
			}	
			if (!IS_NPC(ch) && has_ability(ch, ABIL_CLAWS) && w_type == TYPE_VAMPIRE_CLAWS) {
				if (can_gain_exp_from(ch, victim)) {
					gain_ability_exp(ch, ABIL_CLAWS, 2);
				}
				run_ability_hooks(ch, AHOOK_ABILITY, ABIL_CLAWS, get_ability_level(ch, ABIL_CLAWS), victim, NULL, NULL, NULL);
			}
			...
			if (!IS_NPC(ch) && has_ability(ch, ABIL_BIG_GAME_HUNTER) && (IS_VAMPIRE(victim) || IS_MAGE(victim))) {
				...
				run_ability_hooks(ch, AHOOK_ABILITY, ABIL_BIG_GAME_HUNTER, get_ability_level(ch, ABIL_BIG_GAME_HUNTER), victim, NULL, NULL, NULL);
...
void perform_violence_missile(char_data *ch, obj_data *weapon) {
	...
	if (block) {
		...
		if (!IS_NPC(ch) && has_ability(ch, ABIL_BOWMASTER)) {
			...
			run_ability_hooks(ch, AHOOK_ABILITY, ABIL_BOWMASTER, get_ability_level(ch, ABIL_BOWMASTER), vict, NULL, NULL, NULL);
		}
		...
		if (has_ability(ch, ABIL_QUICK_DRAW)) {
			run_ability_hooks(ch, AHOOK_ABILITY, ABIL_QUICK_DRAW, get_ability_level(ch, ABIL_QUICK_DRAW), vict, NULL, NULL, NULL);
		}
		if (has_ability(ch, ABIL_TRICK_SHOTS)) {
			run_ability_hooks(ch, AHOOK_ABILITY, ABIL_TRICK_SHOTS, get_ability_level(ch, ABIL_TRICK_SHOTS), vict, NULL, NULL, NULL);
		}
	}
