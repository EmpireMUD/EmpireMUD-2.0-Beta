		...
		for (node = node_list; node; node = next_node) {
			...
			if (over_count) {
				...
				if (node->veh) {	// scanned for vehicle
					lsize += snprintf(line + lsize, sizeof(line) - lsize, "%s / %s", skip_filler(VEH_SHORT_DESC(veh)), (GET_BUILDING(node->loc) ? GET_BLD_NAME(GET_BUILDING(node->loc)) : GET_SECT_NAME(SECT(node->loc))));
				}
				else {	// not a vehicle
