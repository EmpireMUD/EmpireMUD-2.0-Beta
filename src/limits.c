...
bool point_update_player(char_data *ch) {
	...
	if (GET_MOUNT_LIST(ch)) {
		...
		run_ability_hooks(ch, AHOOK_ABILITY, ABIL_STABLEMASTER, get_ability_level(ch, ABIL_STABLEMASTER), ch, NULL, NULL, NULL);
	}
		...
		if (has_ability(ch, ABIL_SOLAR_POWER)) {
			if (IS_CLASS_ABILITY(ch, ABIL_SOLAR_POWER) || check_sunny(IN_ROOM(ch))) {
				...
				if (!info_only) {
					...
					run_ability_hooks(ch, AHOOK_ABILITY, ABIL_SOLAR_POWER, get_ability_level(ch, ABIL_SOLAR_POWER), ch, NULL, NULL, NULL);
				}
			}
		}
