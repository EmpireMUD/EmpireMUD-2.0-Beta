...
void olc_delete_object(char_data *ch, obj_vnum vnum) {
	...
	HASH_ITER(hh, progress_table, prg, next_prg) {
		...
		found |= delete_requirement_from_list(&PRG_TASKS(prg), REQ_EMPIRE_GATHER_TOTAL, vnum);
	...
	HASH_ITER(hh, quest_table, quest, next_quest) {
		...
		found |= delete_requirement_from_list(&QUEST_TASKS(quest), REQ_EMPIRE_GATHER_TOTAL, vnum);
		found |= delete_requirement_from_list(&QUEST_PREREQS(quest), REQ_EMPIRE_GATHER_TOTAL, vnum);
	...
	HASH_ITER(hh, social_table, soc, next_soc) {
		...
		found |= delete_requirement_from_list(&SOC_REQUIREMENTS(soc), REQ_EMPIRE_GATHER_TOTAL, vnum);
	...
	for (desc = descriptor_list; desc; desc = desc->next) {
		...
		if (GET_OLC_PROGRESS(desc)) {
			...
			found |= delete_requirement_from_list(&PRG_TASKS(GET_OLC_PROGRESS(desc)), REQ_EMPIRE_GATHER_TOTAL, vnum);
		...
		if (GET_OLC_QUEST(desc)) {
			...
			found |= delete_requirement_from_list(&QUEST_TASKS(GET_OLC_QUEST(desc)), REQ_EMPIRE_GATHER_TOTAL, vnum);
			found |= delete_requirement_from_list(&QUEST_PREREQS(GET_OLC_QUEST(desc)), REQ_EMPIRE_GATHER_TOTAL, vnum);
		...
		if (GET_OLC_SOCIAL(desc)) {
			...
			found |= delete_requirement_from_list(&SOC_REQUIREMENTS(GET_OLC_SOCIAL(desc)), REQ_EMPIRE_GATHER_TOTAL, vnum);
...
void olc_search_obj(char_data *ch, obj_vnum vnum) {
	...
	HASH_ITER(hh, progress_table, prg, next_prg) {
		...
		any |= find_requirement_in_list(PRG_TASKS(prg), REQ_EMPIRE_GATHER_TOTAL, vnum);
	...
	HASH_ITER(hh, quest_table, quest, next_quest) {
		...
		any |= find_requirement_in_list(QUEST_TASKS(quest), REQ_EMPIRE_GATHER_TOTAL, vnum);
		any |= find_requirement_in_list(QUEST_PREREQS(quest), REQ_EMPIRE_GATHER_TOTAL, vnum);
	...
	HASH_ITER(hh, social_table, soc, next_soc) {
		...
		any |= find_requirement_in_list(SOC_REQUIREMENTS(soc), REQ_EMPIRE_GATHER_TOTAL, vnum);
